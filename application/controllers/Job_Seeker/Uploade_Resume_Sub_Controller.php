...
class Uploade_Resume_Sub_Controller extends CI_Controller {
public function __construct()
{
  ...
  //  $this->load->view('Job_Seeker/Job_Seeker');
   ...
   public function seeker_experience(){
	  ...
	  $data=$this->Uploade_Resume_Sub_Model->add_seeker_experience_expert_ifno($userdata);
	   
	   
/* 	  $data=json_decode($_GET['data']);
	   $DataExpert=json_decode($_GET['Expert']);
   $experience = array();
	  // print_r($data);
	   for($i = 0; $i<count($data);$i++)
	   {
		 $temp = explode(",",$data[$i]);
		 array_push($experience,$temp[1]);
	   }
	  ...
	  $expert = array();
	  // print_r($data);
	   for($i = 0; $i<count($DataExpert);$i++)
	   {
		 $temp = explode(",",$DataExpert[$i]);
		 array_push($expert,$temp[1]);
	   }
	   $expert = array_unique($expert);
	  ...
	  for($j = 0;$j<count($DataExpert);$j++)
	  {
		  $temp = explode(",",$DataExpert[$j]);
		 // print_r($temp);
	...
	//  print_r($final);
	$exp=json_encode($final);		
	$expert_da=json_encode($final_Ex);
    $seeker_id=$this->session->userdata('user_id'); 	
	 $userdata=array(
	                   "seeker_id"=>$seeker_id,
	                   "experience"=>$exp,
